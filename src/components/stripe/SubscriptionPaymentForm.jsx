...
export default function SubscriptionPaymentForm ({ priceId }) {
  ...
  useEffect(() => {
    const go = async () => {
      ...
      setLoading(true)
      const r = await fetch('/api/stripe/create-setup-intent', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
function Inner ({ clientSecret, loading, setLoading }) {
  const stripe  = useStripe()
  const elements = useElements()
  const router  = useRouter()
  ...
  const submit = async e => {
    ...
    // If Stripe did not redirect (no 3-DS needed) we route manually:
    if (setupIntent.status === 'succeeded')
      router.push(`/payment-success?setup_intent_client_secret=${setupIntent.client_secret}`)
  }
