...
export async function POST (req) {
  ...
  try {
    ...
    if (!userToken)
      return NextResponse.json(
        { error: 'User token data not found. Refresh and try again.' },
        { status: 403 }
      )

    // ──────────────────────────── 2. Daily reset + expiry checks
    const now          = new Date()
    ...
    if (userToken.unlimitedAccessUntil && now > userToken.unlimitedAccessUntil) {
    ...
    // ──────────────────────────── 4. Unlimited vs quota check
    const hasUnlimitedAccess =
      userToken.maxTokensPerDay === Infinity ||
      (userToken.unlimitedAccessUntil &&
        now <= userToken.unlimitedAccessUntil)

    if (
      !hasUnlimitedAccess &&
      userToken.tokensUsedToday + tokensNeeded > userToken.maxTokensPerDay
    ) {
      return NextResponse.json(
        { error: `Daily token limit exceeded. Needed ${tokensNeeded}, but you only have ${userToken.maxTokensPerDay - userToken.tokensUsedToday}.` },
        { status: 429 }
      )
    }

    // ──────────────────────────── 5. Validate files + format
    if (!files.length)
      return NextResponse.json({ error: 'No image files provided.' }, { status: 400 })

    const supportedFormats = ['jpeg', 'jpg', 'png', 'webp', 'avif', 'tiff', 'raw', 'gif']
    if (!supportedFormats.includes(outputFormat))
      return NextResponse.json(
    ...
    const maxBytes = maxFileSize ? parseInt(maxFileSize, 10) * 1024 * 1024 : null
    for (const f of files)
      if (maxBytes && f.size > maxBytes)
        return NextResponse.json(
          { error: `File "${f.name}" exceeds the size limit.` },
          { status: 413 }
        )

    // ──────────────────────────── 6. Convert images with Sharp
    const results = []
    for (const file of files) {
      try {
        ...
        switch (outputFormat) {
          case 'jpeg':
          case 'jpg':
            outBuf = await s.jpeg({ quality: 80 }).toBuffer()
            mime = 'image/jpeg'
            break
          case 'png':
            outBuf = await s.png({ compressionLevel: 9 }).toBuffer()
            mime = 'image/png'
            break
          case 'webp':
            outBuf = await s.webp({ quality: 80 }).toBuffer()
            mime = 'image/webp'
            break
          case 'avif':
            outBuf = await s.avif({ quality: 70 }).toBuffer()
            mime = 'image/avif'
            break
          case 'tiff':
            outBuf = await s.tiff().toBuffer()
            mime = 'image/tiff'
            break
          case 'gif':
            outBuf = await s.gif().toBuffer()
            mime = 'image/gif'
            break
          case 'raw':
            outBuf = await s.raw().toBuffer()
    ...
    // ──────────────────────────── 7. Token consumption
    if (!hasUnlimitedAccess && successful) {
      userToken.tokensUsedToday += successful
      await userToken.save()
    }
    ...
    const updatedQuota = {
      tokensUsed: userToken.tokensUsedToday,
      maxTokens:  hasUnlimitedAccess ? Infinity : userToken.maxTokensPerDay,
      unlimitedAccessUntil: userToken.unlimitedAccessUntil ?? null
    }

    // ──────────────────────────── 8. Return response
    const res = NextResponse.json(
      { convertedImages: results, ...updatedQuota },
      { status: 200 }
    )
    res.headers.set('X-Tokens-Updated', JSON.stringify(updatedQuota))
    return res
  } catch (err) {
    console.error('[convert-image] Fatal error:', err)
    return NextResponse.json(
